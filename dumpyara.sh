...
if [[ -n $GIT_OAUTH_TOKEN ]]; then
    ...
    if [[ -z "$(git config --get user.email)" ]]; then
        git config user.email anggit.febriantoro@gmail.com
    fi
    if [[ -z "$(git config --get user.name)" ]]; then
        git config user.name a9ito
    fi
    git checkout -b "$branch"
    find . -size +97M -printf '%P\n' -o -name "*sensetime*" -printf '%P\n' -o -name "*.lic" -printf '%P\n' >| .gitignore
    git add --all
    git remote add origin https://oauth2:"$GIT_OAUTH_TOKEN"@git.rip/a9ito/dumpyara.git
    git commit -asm "Add ${description}"
    git push https://oauth2:"$GIT_OAUTH_TOKEN"@git.rip/a9ito/dumpyara.git "$branch" ||
        (
...
if [[ -n "$TG_TOKEN" ]]; then
    ...
    printf "<b>Brand: %s</b>" "$brand" >| "$PROJECT_DIR"/working/tg.html
    {
        ...
        printf "\n<b>GitHub:</b>"
        printf "\n<a href=\"%s\">Branch</a>" "https://git.rip/a9ito/dumpyara/-/tree/"$branch""
